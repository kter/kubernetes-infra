...
(こちら)[https://github.com/prabhatsharma/kubernetes-the-hard-way-aws/blob/master/docs/04-certificate-authority.md]の指示に従い発行
...
```
  instance="worker-1"
  instance_hostname="ip-10-1-0-250"
...
EOF
  ...
  instance="worker-2"
  instance_hostname="ip-10-1-2-250"
  cat > ${instance}-csr.json <<EOF
{
  "CN": "system:node:${instance_hostname}",
  "key": {
...
cfssl gencert \
  ...
  -ca-key=ca-key.pem \
  -config=ca-config.json \
  -hostname=10.32.0.1,10.1.0.250,10.1.2.250,${KUBERNETES_PUBLIC_ADDRESS},127.0.0.1,${KUBERNETES_HOSTNAMES} \
  -profile=kubernetes \
...
パラメータストアから秘密鍵をダウンロードして`kubernetes.id_rsa`にリネームする
